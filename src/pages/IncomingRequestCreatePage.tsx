...
const IncomingRequestCreatePage: React.FC = () => {
  ...
  const handleCampaignChange = useCallback((keys: SharedSelection) => {
    const value = Array.from(keys)[0];
    setValue('advertising_campaign_id', value ? Number(value) : undefined);
  }, [setValue]);

  const handleCityChange = useCallback((keys: SharedSelection) => {
    const cityId = Array.from(keys)[0];
    setValue('city_id', Number(cityId));
  }, [setValue]);

  const handleRequestTypeChange = useCallback((keys: SharedSelection) => {
    const typeId = Array.from(keys)[0];
    setValue('request_type_id', Number(typeId));
  }, [setValue]);

  const handleDirectionChange = useCallback((keys: SharedSelection) => {
    const value = Array.from(keys)[0];
