...
const CallCenterReportPage: React.FC = () => {
  ...
  const stats = useMemo<CallCenterStats>(() => {
    ...
    const completedRequests = filteredRequests.filter(r => 
      ['Готово'].includes(r.status || '')
    ).length;
    const pendingRequests = filteredRequests.filter(r => 
      ['Новая', 'Ожидает', 'Ожидает принятия'].includes(r.status || '')
    ).length;
        ...
        <CardBody>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            >
              ...
              <option value="Ожидает принятия">Ожидает принятия</option>
              ...
              <option value="НеЗаказ">НеЗаказ</option>
              <option value="Перезвонить">Перезвонить</option>
              <option value="ТНО">ТНО</option>
            </select>
            <Input
              type="date"
              label="Дата от"
