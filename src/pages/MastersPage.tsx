...
const MastersPage: React.FC = () => {
  ...
  const filteredMasters = useMemo(() => {
    ...
    return mastersData.filter(master => {
      ...
      const matchesCity = filters.city === 'all' || (master.city?.id === parseInt(filters.city));
   ...
   const masterStats = useMemo(() => {
     ...
     const uniqueCities = new Set(mastersData.map(m => m.city?.id).filter(Boolean));
  ...
  const getCityName = useCallback((city: { id: number; name: string } | null | undefined) => {
